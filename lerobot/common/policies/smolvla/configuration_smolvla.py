...
class SmolVLAConfig(PreTrainedConfig):
    ...
    num_vlm_layers: int = (
        16  # Number of layers used in the VLM (first num_vlm_layers layers)
    )
    self_attn_every_n_layers: int = (
        2  # Interleave SA layers each self_attn_every_n_layers
    )
    expert_width_multiplier: float = (
        0.75  # The action expert hidden size (wrt to the VLM)
    )
